...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($username != "" && $password != "" && $password1!="") {
        if($password==$password1){
            $res = register_auth($username, hash('sha256',$password), $conn);
            if ($res != "OK") {
                $_SESSION["regis_err"] = $res;
            } else { 
                header("Location: /chats.php");
            }
        }
        else{
            $_SESSION["regis_err"] = "Пароли не совпадают";
        }
    } else {
        $_SESSION["regis_err"] = "Заполните все поля";
    }
}
...
<body class="dark:bg-zinc-900">
    ...
    <div class="container mx-auto px-4">
        ...
        <form method="POST" class="my-4">
            ...
            <div class="mb-6">
                <label for="password1" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Repeat password</label>
                <input name="password1" minlength="8" maxlength="64" type="password" id="password1" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="********************" required>
            </div>
            <button type="submit" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Submit</button>
        </form>
        <a href="/login.php" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Have a account?</a>
        <?php
        if (isset($_SESSION["regis_err"])) {
            echo "<h2 class='dark:text-red-600 text-red-700'>" . $_SESSION["regis_err"] .  "</h2>";
            $_SESSION["regis_err"] = "";
        }
        ?>
    </div>
